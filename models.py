...
class AttendanceReport(models.Model):
    ...
    def save(self,*args, **kwargs):        
        ...
        """
        subject = 'Record of your attendance'
        email_message = render_to_string('attendance.html',{                
        'name': self.student.user.first_name +" "+ self.student.user.last_name,
        "unit":self.attendance.unit.name,
        "date":self.attendance.attendance_date,
        })        
        receivers = [f"{self.student.user.email}"]
        mail = EmailMessage(subject, email_message, f'<{settings.EMAIL_HOST_USER}>', bcc=receivers)  
        mail.send()
        return super().save(*args, **kwargs)
...
class StudentResult(models.Model):
    ...
    def save(self, *args, **kwargs):        
        ...
        """
        subject = 'Record of your marks'
        email_message = render_to_string('marks.html',{                
        'name': self.student.user.first_name +" "+ self.student.user.last_name,
        "unit":self.unit.name,
        "exam_marks":self.unit_exam_marks,
        "assignment_marks":self.unit_assignment_marks,
